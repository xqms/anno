...
template<template<auto ...> class T>
constexpr detail::template_type_nontype_args<T> type()
...
struct AnnotationList
{
    ...
    static consteval auto get()
    {
        ...
        static_assert(size <= 1, "Called get() on an AnnotationList with more than one annotation");
...
struct Member
{
    ...
    constexpr Annotations annotations() const
    { return Annotations{}; }
...
#define ANNO_CUSTOM(init_code, line, ...) decltype([](){ init_code; return anno::AnnotationList<__VA_ARGS__>(); }()) ANNO_CONCAT(zzz_anno, line) [[no_unique_address]];
...
namespace detail
{
    ...
    constexpr auto operator+(const IndexSeq<VA...>&, const wrapConcat<IndexSeq<VB...>>&) noexcept
    {
    ...
    {
        if constexpr(requires { []<auto ... A>(const AnnotationList<A...>&){}(T{}); })
            return true;
        else
            return false;
    }
...
constexpr auto members(const Struct& s = {})
{
    // Get indices of all real members (not annotations)
    auto memberIndices = [&]<auto ... Ns>(detail::IndexSeq<Ns...>) {
        return detail::concat<detail::IndexSeq,
            std::conditional_t<
                detail::isAnnotationList<std::remove_cvref_t<decltype(reflect::get<Ns>(s))>>(),
                detail::IndexSeq<>,
                detail::IndexSeq<Ns>
            >...>();
    ...
    auto getMember = []<int annotationStart, int memberIndex>(std::integral_constant<int, annotationStart>, std::integral_constant<int, memberIndex>){

        auto annotations = [&]<auto ... Ns>(std::index_sequence<Ns...>) {
            using Annotations = detail::concatenate_t<AnnotationList,
                std::conditional_t<
                    detail::isAnnotationList<std::remove_cvref_t<decltype(reflect::get<annotationStart + Ns>(s))>>(),
                    std::remove_cvref_t<decltype(reflect::get<annotationStart + Ns>(s))>,
                    AnnotationList<>
                >...
            >;
            return Annotations();
        }(std::make_index_sequence<(memberIndex >= annotationStart) ? (memberIndex-annotationStart) : 0>());

        return Member<Struct, static_cast<std::size_t>(memberIndex), decltype(annotations)>();
    };

    return [&]<auto ... startIndex, auto... memberIndex>(detail::IndexSeq<startIndex...>, detail::IndexSeq<memberIndex...>) {
        return detail::concat<MemberList,
            std::conditional_t<memberIndex >= 0,
                MemberList<decltype(getMember(std::integral_constant<int, startIndex>(), std::integral_constant<int, memberIndex>()))>,
                MemberList<>
            >...
        >();
...
template<auto... Anns>
template<auto F>
constexpr auto AnnotationList<Anns...>::filter()
{
    using Matches = detail::concatenate_t<AnnotationList,
        std::conditional_t<
            F(Anns),
            AnnotationList<Anns>,
            AnnotationList<>
...
namespace tests
{
    ...
    static_assert(sizeof(Test) == 2, "Annotations should not increase struct size");
    static_assert(std::is_same_v<
        anno::members_t<Test>,
        MemberList<
            Member<Test, 1, AnnotationList<anns::Help{"my help string"}, anns::Short{'h'}>>,
            Member<Test, 4, AnnotationList<
                anns::Help{"verbose"},
                anns::Short{'v'},
                anns::Short{'d'}
            >>
    ...
    static_assert([](){
        ...
        anno::members<Test>().for_each([&](const auto& member){
            member.annotations().for_each([&](const auto& ann){
                annotationCounter++;
            });
    ...
    static_assert([](){
        auto helpAnn = anno::members_t<Test>::Member<0>::Annotations::filter(anno::type<anns::Help>()).get();
